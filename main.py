...
def get_scores_for_workflow(answers: Dict[str, str]) -> Dict[str, int]:
    ...
    if yes_no(answers.get("workflow_active")) is False:
        scores["workflow_inactive"] += 100

    if yes_no(answers.get("event_happened")) is False:
        scores["trigger_event_missing"] += 80

    if yes_no(answers.get("used_real_contact")) is False:
        scores["trigger_event_missing"] += 25
        scores["trigger_conditions_not_met"] += 10

    if yes_no(answers.get("has_filters")) is True:
        scores["trigger_conditions_not_met"] += 35

    if yes_no(answers.get("tag_used_in_trigger")) is True:
        scores["wrong_field_or_tag"] += 20

    if yes_no(answers.get("tag_used_in_trigger")) is True and yes_no(answers.get("tag_confirmed")) is False:
        scores["wrong_field_or_tag"] += 60

    if yes_no(answers.get("other_workflows_exist")) is True:
        scores["conflicting_automation"] += 40

    if yes_no(answers.get("worked_before")) is True and yes_no(answers.get("recent_changes")) is True:
        scores["workflow_recent_change_breakage"] += 55

    return scores


def get_scores_for_email_sms(answers: Dict[str, str]) -> Dict[str, int]:
    scores = {
        "campaign_inactive": 0,
        "missing_template": 0,
        "sender_not_connected": 0,
        "contact_opted_out": 0,
        "sms_setup_or_credits_missing": 0,
        "sending_window_blocking": 0,
        "message_step_never_reached": 0,
        "duplicate_or_conflicting_automation": 0,
    }

    channel = answers.get("channel")

    if yes_no(answers.get("workflow_live")) is False:
        scores["campaign_inactive"] += 100

    if yes_no(answers.get("template_selected")) is False:
        scores["missing_template"] += 85

    if yes_no(answers.get("sender_connected")) is False:
        scores["sender_not_connected"] += 90

    if yes_no(answers.get("contact_opted_in")) is False:
        scores["contact_opted_out"] += 70

    if channel in {"sms", "both"} and yes_no(answers.get("has_sms_credits")) is False:
        scores["sms_setup_or_credits_missing"] += 85

    if yes_no(answers.get("uses_delays_windows")) is True:
        scores["sending_window_blocking"] += 45

    if yes_no(answers.get("contact_reached_step")) is False:
        scores["message_step_never_reached"] += 95

    if yes_no(answers.get("multiple_automations")) is True:
        scores["duplicate_or_conflicting_automation"] += 45

    return scores


def get_scores_for_calendar(answers: Dict[str, str]) -> Dict[str, int]:
    scores = {
        "no_availability": 0,
        "wrong_calendar_assignment": 0,
        "timezone_issue": 0,
        "form_handoff_failed": 0,
        "confirmation_workflow_broken": 0,
        "routing_logic_issue": 0,
    }

    symptom = answers.get("calendar_symptom")

    if yes_no(answers.get("availability_set")) is False:
        scores["no_availability"] += 100

    if yes_no(answers.get("calendar_connected")) is False:
        scores["wrong_calendar_assignment"] += 90

    if yes_no(answers.get("timezone_correct")) is False:
        scores["timezone_issue"] += 65

    if yes_no(answers.get("uses_form_handoff")) is True and yes_no(answers.get("form_submits_ok")) is False:
        scores["form_handoff_failed"] += 90

    if symptom == "no_confirmation":
        scores["confirmation_workflow_broken"] += 85

    if yes_no(answers.get("round_robin")) is True:
        scores["routing_logic_issue"] += 55

    if symptom == "wrong_user":
        scores["wrong_calendar_assignment"] += 45
        scores["routing_logic_issue"] += 35

    if symptom == "double_booking":
        scores["routing_logic_issue"] += 45
        scores["timezone_issue"] += 15

    if yes_no(answers.get("booking_visible_in_calendar")) is False:
        scores["wrong_calendar_assignment"] += 20
        scores["form_handoff_failed"] += 20

    return scores


def score_issue(issue_type: str, answers: Dict[str, str]) -> Dict[str, int]:
    if issue_type == "workflow_not_firing":
        return get_scores_for_workflow(answers)
    if issue_type == "email_sms_not_sending":
        return get_scores_for_email_sms(answers)
    if issue_type == "calendar_issue":
        return get_scores_for_calendar(answers)
    raise HTTPException(status_code=400, detail="Unsupported issue type")


def determine_confidence(top_score: int, second_score: int) -> str:
...
def build_report_text(issue_type: str, primary: DiagnosisDetail, secondary: List[SecondaryCause]) -> str:
    lines = [
        "Support Troubleshoot Wizard Report",
        "",
        f"Issue Type: {issue_type}",
        f"Primary Diagnosis: {primary.title}",
        f"Confidence: {primary.confidence}",
        f"Severity: {primary.severity}",
        "",
        "Why this is likely:",
    ]
    lines.extend([f"- {item}" for item in primary.why])
    lines.append("")
    lines.append("Fix steps:")
    lines.extend([f"{idx + 1}. {step}" for idx, step in enumerate(primary.fix_steps)])

    if secondary:
        lines.append("")
        lines.append("Secondary possible causes:")
        lines.extend([f"- {item.title}: {item.hint}" for item in secondary])

    return "\n".join(lines)


def build_diagnosis_response(issue_type: str, answers: Dict[str, str]) -> DiagnoseResponse:
    scores = score_issue(issue_type, answers)
    ranked = sorted(scores.items(), key=lambda x: x[1], reverse=True)

    if not ranked or ranked[0][1] <= 0:
        fallback_code = {
            "workflow_not_firing": "trigger_conditions_not_met",
            "email_sms_not_sending": "message_step_never_reached",
            "calendar_issue": "wrong_calendar_assignment",
        }[issue_type]
        ranked = [(fallback_code, 10)]

    top_code, top_score = ranked[0]
    second_score = ranked[1][1] if len(ranked) > 1 else 0
    confidence = determine_confidence(top_score, second_score)

    primary_lib = DIAGNOSIS_LIBRARY[top_code]
    primary = DiagnosisDetail(
        code=top_code,
        title=primary_lib["title"],
        confidence=confidence,
        severity=primary_lib["severity"],
        why=primary_lib["why"],
        fix_steps=primary_lib["fix_steps"],
        retest_steps=primary_lib["retest_steps"],
    )

    secondary: List[SecondaryCause] = []
    for code, score in ranked[1:4]:
        if score > 0:
            lib = DIAGNOSIS_LIBRARY[code]
            secondary.append(
                SecondaryCause(
                    code=code,
                    title=lib["title"],
    ...
    report_text = build_report_text(issue_type, primary, secondary)
